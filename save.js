...
function select_annotation(elemnt) {
    error_signal()
    if (ann_state === "create") {
        ann_cur_id = elemnt.parentElement.value
...
function remove_annotation(elemnt) {
    error_signal()
    if (ann_state === "create") {
        selected_id = elemnt.parentElement.value
...
function add_ann_tool() {
    $('#ann_'+ann_cur_id).append('<a href="#" onclick="select_annotation(this)" style="margin-left: 70px">' + "✂" + '</a>')
    $('#ann_'+ann_cur_id).append('<a href="#" onclick="remove_annotation(this)" style="margin-left: 15px">' + "⤺" + '</a>')     
}
...
Mousetrap.bind('q', () => {
    ...
    if (ann_state === "modify" || ann_state === "create") {
        ...
        if (ann_state === "create") {
            ...
            ann_state = "modify"
            // create ann bar
            slider_bar.append('<div class="vjs-ann-bar" id="ann_bar_'+ ann_cur_id + '"></div>')
            set_ann_bar(ann_now["start"], ann_now["start"]+0.15, ann_cur_id, color='#808080')
        } else {
            ...
            if ((ann_now["end"] === 0 && currentTime > 0) || (currentTime < ann_now["end"])) {
                ...
                if (ann_now["end"] === 0) {
                    ...
                    add_ann_tool()
                    set_ann_bar(ann_now["start"], ann_now["end"], ann_cur_id, color=color_now)
                }
            }
        }
    }
